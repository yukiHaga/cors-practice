...
var Routing = []*pattern.URLPattern{
	...
	pattern.NewURLPattern("/same-origin-policy", controller.NewSameOriginPolicy()),
	pattern.NewURLPattern("/preflight-request", controller.NewPreflightRequest()),
	pattern.NewURLPattern("/authentication-included-request", controller.NewAuthenticationIncludedRequest()),
}
